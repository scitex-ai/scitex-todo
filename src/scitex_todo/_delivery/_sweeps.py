...
:mod:`scitex_todo._delivery._daemon` owns the loop (single-instance lock,
...
* :func:`_run_reminder_sweep` — every tick: enqueue any DUE owner digests +
  escalations (:func:`scitex_todo._reminders.sweep_reminders`) so the SAME
...
def _run_stale_nudge_sweep(*, store, now) -> None:
    ...
    Runs :func:`scitex_todo._stale_active_nudge.sweep_and_nudge`, which is
