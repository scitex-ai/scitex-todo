...
---------------------------------------
The write chokepoint (:func:`scitex_cards._model._save_doc_unlocked`) receives the
...
2. **BUT IT MUST NEVER BE SILENT.** A mirror that fails quietly lets the DB rot out
   ...
   counted, and surfaced in ``scitex-cards health``.
...
def mirror_after_save(doc: dict, store_path: str | Path) -> bool:
    ...
    try:
        ...
        logger.error(
            ...
            "(`scitex-cards db import`) before S2 cutover; do NOT trust it until "
            "then. Store: %s",
            len(_failures),
            msg,
            store_path,
...
def check_mirror_healthy() -> dict[str, object]:
    ...
    return {
        ...
        "hint": (
            ...
            "from the YAML: `scitex-cards db import`. Then investigate why the "
