...
def test_add_task_appends_preserves_order(tmp_path):
    ...
    _store.add_task(store, id="b", title="B", status="in_progress",
                    scope="agent:proj-scitex-cards", assignee="agent:proj-scitex-cards",
...
def test_add_task_accepts_project_via_extras(tmp_path):
    ...
    store = tmp_path / "tasks.yaml"
    # Act
    _store.add_task(store, id="a", title="A", project="scitex-cards", assignee="agent:test-suite")
...
def test_update_task_passing_none_clears_field_in_return(tmp_path):
    # Arrange
    store = tmp_path / "tasks.yaml"
    _store.add_task(store, id="a", title="A", scope="agent:proj-scitex-cards", assignee="agent:test-suite")
    # Act
