...
C2 adds the canonical ``card-event`` kind (C1's :mod:`scitex_todo._events`
envelope) to the validator: a producer that emits a typed
:class:`scitex_todo._events.Event` via ``emit`` now passes validation
instead of being rejected as an unknown kind. The inner ``type`` is
validated against :data:`scitex_todo._events.EVENT_TYPES` — reused, NOT
...
#:                      (:mod:`scitex_todo._events`). The inner ``type``
#:                      is one of :data:`scitex_todo._events.EVENT_TYPES`
...
def event_validate(event: Any) -> dict:
    ...
    if kind == "push":
        ...
        # Canonical C1 envelope (scitex_todo._events.Event.to_dict()).
