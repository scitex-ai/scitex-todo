...
hook bus (:func:`scitex_todo._hooks.dispatch_event`). It is **pure model
...
:mod:`scitex_todo._hooks` already fans a *dict* event to built-in
handlers + ``scitex_todo.hooks`` entry-point plugins; ``comment_task``
...
class Event:
    ...
    ts : str
        ...
        :func:`scitex_todo._store._utc_now_iso` when not supplied, so two
