...
then calls ``scitex_todo.save_tasks`` to round-trip the YAML store (preserving
...
def handle_priority(request, board):
    ...
    if request.method != "POST":
        return JsonResponse(
            {"error": "priority endpoint requires POST"}, status=405
        )
    ...
    return JsonResponse(
        {"updated": updated, "store_path": str(board.store_path)}
    )
