...
1. **Store resolution.** Default precedence: `--tasks` flag → `$SCITEX_TODO_TASKS_YAML_SHARED` → project `<git-root>/.scitex/todo/tasks.yaml` → user `~/.scitex/todo/tasks.yaml` → bundled example. Check with `scitex-cards resolve-store`. Many agents bind only the user store; the project store can shadow it silently.
