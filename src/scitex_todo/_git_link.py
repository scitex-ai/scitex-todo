...
to ``scitex-todo hook push`` (see :mod:`scitex_todo._hooks` for the wire
...
def build_push_event(
    *,
    ...
    :func:`scitex_todo._hooks.event_validate`.
...
def _main(argv: list[str] | None = None) -> int:
    ...
    ``python -m scitex_todo._git_link card-id <branch> [<commit-msg-file>]``
    ...
    if mode == "emit-event":
        parser = argparse.ArgumentParser(prog="scitex_todo._git_link emit-event")
