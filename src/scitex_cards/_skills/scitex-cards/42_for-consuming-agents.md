...
```bash
# 1. Confirm scitex-cards is installed + which tier it resolves to.
scitex-cards --version
scitex-cards resolve-store                  # prints the resolved path + chain
...
`scitex-cards` resolves the store in this order; first existing wins:
...
scitex-cards --tasks ~/.scitex/todo/tasks.yaml add fleet-cutover-2026Q3 \
    '[P0] [GOAL] Fleet release cutover 2026Q3' ...

# Or env-pin for a session (useful in scripts):
SCITEX_TODO_TASKS_YAML_SHARED=~/.scitex/todo/tasks.yaml scitex-cards list-tasks --json
```
...
- `assignee` (str) — **PRIMARY agent-linking field. Set this to YOUR
  agent name** (e.g. `scitex-cards`). The lead's empirical
  dogfood (2026-06-07) confirms `scitex-cards list-tasks --assignee
...
```bash
# Lead seeding a task into the scitex-cards project tier:
scitex-cards --tasks ~/proj/scitex-cards/.scitex/todo/tasks.yaml \
  add scitex-cards-fleet-rollout \
  '[P0] Fleet rollout of scitex-cards skill across agents' \
  --scope agent:scitex-cards \
  --assignee scitex-cards \
  --status pending
```
...
- **Pushes** a sac channel event on `scitex-cards:task:<project>/<id>`
...
- **Subscribes** to the `scitex-cards:task:*` firehose on the sac
...
until the consumer pip-bumps `scitex-cards`. That gives the lead a
