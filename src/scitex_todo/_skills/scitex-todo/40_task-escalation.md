---
description: |
  [TOPIC] Backlog burn-down — blocker-driven scheduling on the shared board
  ...
  lead-driven escalation cycle dispatches it). Each sweep (1) re-checks
  every BLOCKED task to see if its blocker has cleared (auto-unblock),
  then (2) escalates every RUNNABLE task to the lead via a2a so the lead
  can dispatch the owning agent. Goal: keep CONSUMPTION rate > ARRIVAL
  rate so the board doesn't drift from the live codebase.
  [HOW] Sweep the YAML, partition into BLOCKED vs RUNNABLE, re-check
  ...
  RUNNABLE work OUT.
tags: [scitex-todo-task-escalation, scitex-todo-burn-down, scitex-todo-blockers]
---

# Backlog burn-down — blocker-driven scheduling
...
1. **`compute`**: is the `depends_on:` job/task still pending? Query
   ...
   `"[burn-down sweep YYYY-MM-DD] unblocked — compute dependency
   <id> resolved."`
...
(`~/proj/scitex-lead/GITIGNORED/RUNNING/burn-down.md` or equivalent):
...
```
[burn-down 2026-06-08] sweep N=267 → unblocked=2 dispatched=24 awaiting=3 net=-1
```
