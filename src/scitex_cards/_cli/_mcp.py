...
scitex-cards's OWN feature (no scitex-dev parallel) and is wired onto the
group in BOTH paths.
...
def _fallback_mcp_group() -> click.Group:
    ...
    # ── channel (scitex-cards's own standalone server) ─────────────────── #
...
def register(main: click.Group) -> None:
    ...
    try:
        ...
        # Override scitex-dev's tools-only `start` with scitex-cards's UNIFIED
        # server (tools + digest push) — one `scitex-cards` MCP integration.
        _attach_unified_start(mcp_group)
        # scitex-cards's OWN channel verb has no scitex-dev parallel — wire it
