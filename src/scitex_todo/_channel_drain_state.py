...
The channel poll loop (:func:`scitex_todo._mcp_channel._poll_loop`) used to call
:func:`scitex_todo._mcp_channel.drain_once` every ``_DEFAULT_INTERVAL`` (5s)
...
class _DrainState:
    ...
    Mirrors :class:`scitex_todo._wake_watcher.WatcherState`'s
...
def _resolve_store_file(store: str | Path | None) -> Optional[Path]:
    ...
    :func:`scitex_todo._paths.resolve_tasks_path` (via ``_inbox._resolved_store``);
...
async def gated_drain_once(
    ...
    *,
    ...
    Otherwise it delegates to :func:`scitex_todo._mcp_channel.drain_once`,
