...
prefix (the one :data:`scitex_todo._users.IDENTITY_PREFIXES` exists to STRIP).
...
from scitex_todo._mcp_instructions import build_instructions
...
def _server_instructions_under(env_agent_id: str | None) -> str:
    ...
    proc = subprocess.run(
        [
            sys.executable,
            "-c",
            "from scitex_todo._mcp_server import mcp; print(mcp.instructions)",
        ],
        capture_output=True,
        text=True,
        env=env,
...
def test_no_dead_prefix_anywhere_in_the_mcp_surface():
    ...
    # Arrange
    pytest.importorskip("fastmcp", reason="scitex-todo[mcp] extra not installed")
    from scitex_todo._mcp_server import mcp  # noqa: PLC0415
...
def test_no_dead_proj_identity_examples_in_shipped_surfaces():
    ...
    if not (root / "src" / "scitex_todo").is_dir():  # installed, not a checkout
...
def _registered_tools(mcp):
    ...
    Mirrors the strategy in ``tests/scitex_todo/test__mcp_server.py`` and
