...
fastmcp = pytest.importorskip(
    "fastmcp",
    reason=(
        "fastmcp not installed — `scitex-cards[mcp]` extra not present. "
        "Install with `pip install scitex-cards[mcp]` to run the MCP tests."
    ),
)
...
def test_mcp_server_name():
    # Arrange — MCP tool prefix (mcp__scitex-todo__*) stays unchanged in
    # this pass; that's a separate, later rename with its own alias layer.
    mod = importlib.import_module("scitex_cards._mcp_server")
    # Act
    name = mod.mcp.name
    # Assert
...
def test_scope_filter_excludes_other_scope(tmp_path):
    ...
    # Act
    listed = asyncio.run(
        _call_tool(list_tasks, scope="agent:proj-scitex-cards", tasks_path=store)
    )
    # Assert
