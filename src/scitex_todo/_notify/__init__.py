...
* :mod:`scitex_todo._events` — the canonical :class:`~scitex_todo._events.Event`
  taxonomy (:data:`~scitex_todo._events.EVENT_TYPES`). Recipients are
  resolved per ``event.type``.
* :mod:`scitex_todo._users` — the standalone user registry. All
  name→id resolution goes through :func:`~scitex_todo._users.resolve_user`
  (SSOT for identity); per-user notify prefs live on ``User.notify``.
* :mod:`scitex_todo._model` — the card schema. The role fields are
  ``agent`` (owner), ``assignee``, ``collaborators`` (list),
  ``subscribers`` (list).
* :mod:`scitex_todo._paths` — :func:`~scitex_todo._paths.resolve_tasks_path`
...
``from scitex_todo._notify import resolve_recipients`` works.
"""
