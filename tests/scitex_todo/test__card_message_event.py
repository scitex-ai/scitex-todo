...
def captured_events(monkeypatch):
    sink = _Capturing()
    monkeypatch.setattr(
        hooks_module, "_iter_entry_points",
        lambda: [_FakeEP("captor", sink)],
    )
    return sink.events


def test_comment_task_emits_card_message_event(tmp_path: Path, captured_events):
    # Arrange
    store = tmp_path / "tasks.yaml"
    add_task(store=store, id="c-1", title="x")
    # Act
    comment_task(store=store, task_id="c-1", text="hello", by="operator")
    # Assert
    assert any(e.get("kind") == "card-message" for e in captured_events)


def test_card_message_event_carries_card_id_body_author(
    tmp_path: Path, captured_events,
):
    # Arrange
    store = tmp_path / "tasks.yaml"
    add_task(store=store, id="c-1", title="x")
    ...
    comment_task(store=store, task_id="c-1", text="hello", by="operator")
    # Assert
    e = [x for x in captured_events if x.get("kind") == "card-message"][0]
...
def test_card_message_collaborators_excludes_owner(
    ...
    store = tmp_path / "tasks.yaml"
    add_task(store=store, id="c-1", title="x", agent="proj-clew")
    comment_task(store=store, task_id="c-1", text="first", by="proj-clew")
    captured_events.clear()
    # Act
    comment_task(store=store, task_id="c-1", text="second", by="operator")
    # Assert
    e = [x for x in captured_events if x.get("kind") == "card-message"][0]
...
def test_card_message_collaborators_excludes_new_author(
    ...
    store = tmp_path / "tasks.yaml"
    add_task(store=store, id="c-1", title="x")
    comment_task(store=store, task_id="c-1", text="a", by="operator")
    captured_events.clear()
    # Act
    comment_task(store=store, task_id="c-1", text="b", by="operator")
    # Assert
    e = [x for x in captured_events if x.get("kind") == "card-message"][0]
...
def test_card_message_collaborators_lists_prior_commenters(
    ...
    store = tmp_path / "tasks.yaml"
    add_task(store=store, id="c-1", title="x")
    comment_task(store=store, task_id="c-1", text="a", by="alice")
    comment_task(store=store, task_id="c-1", text="b", by="bob")
    comment_task(store=store, task_id="c-1", text="c", by="alice")  # dedupe
    captured_events.clear()
    # Act
    comment_task(store=store, task_id="c-1", text="d", by="operator")
    # Assert — alice + bob (deduped) appear, in original order.
    e = [x for x in captured_events if x.get("kind") == "card-message"][0]
    assert e["collaborators"] == ["alice", "bob"]


def test_comment_task_save_succeeds_even_when_bus_raises(
    tmp_path: Path, monkeypatch,
):
    # Arrange — plugin raises. Comment must still be on disk.
    store = tmp_path / "tasks.yaml"
    add_task(store=store, id="c-1", title="x")
