...
def register(main: click.Group) -> None:
    ...
    # `summary` is a NOUN, not a verb: it became `cards list --summary`
    # (doctrine §1). `_cli/_cards.py` invokes `summary_cmd` for that mode and
    # owns its Phase-W alias, so it is deliberately not mounted here.
