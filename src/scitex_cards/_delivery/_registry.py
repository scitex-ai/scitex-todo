...
#: LEGACY group name, from before the package was renamed scitex_todo ->
#: scitex_cards. Still scanned: external producers already register their
#: channels under the old group and must keep loading. Entries are merged with
#: the canonical group and de-duplicated by entry-point name (canonical wins).
...
def _load_entry_point_channels() -> list[tuple[str, DeliveryChannel]]:
    ...
    for ep in eps:
        ...
        # The canonical group is scanned first, so a same-named entry point in
        # the legacy group is the SAME provider registered twice (this package
        # registers both) — skip it silently rather than warn on a duplicate.
        if ep_name in seen_ep_names:
            continue
        seen_ep_names.add(ep_name)
        try:
            factory = ep.load()
