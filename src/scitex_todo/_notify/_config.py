...
The config half of foundation C3 (see :mod:`scitex_todo._notify`). Holds the
...
:data:`~scitex_todo._notify._rules.DEFAULT_NOTIFY_RULES` with an optional
...
def notify_sidecar_path(store: str | Path | None) -> Path | None:
    ...
    Reuses :func:`scitex_todo._paths.resolve_tasks_path` so the sidecar
...
def load_notify_config(store: str | Path | None = None) -> NotifyConfig:
    ...
    The built-in :data:`~scitex_todo._notify._rules.DEFAULT_NOTIFY_RULES` is
    ...
    store : str | pathlib.Path | None
        ...
        :func:`scitex_todo._paths.resolve_tasks_path` (the same chain the
