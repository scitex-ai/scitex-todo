...
def register(main: click.Group) -> None:
    """Attach the ``watch`` verb to the root group.

    ``next`` is no longer mounted here: it is a determiner, not a verb, and
    became ``cards list --next`` (doctrine §1). :mod:`scitex_cards._cli._cards`
    invokes ``next_cmd`` for that mode and owns its Phase-W alias.
    """
