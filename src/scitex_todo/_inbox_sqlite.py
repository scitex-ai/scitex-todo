...
digest-poll loop calls :func:`scitex_todo._inbox.poll_inbox` every 5 s — each
...
path in :mod:`scitex_todo._inbox` remains the DEFAULT and is untouched; this
backend is opt-in via ``SCITEX_TODO_INBOX_BACKEND=sqlite`` (the switch lives in
:mod:`scitex_todo._inbox`). Semantics — dedup key ``(event_type, card_id, ts,
...
Connection / schema conventions mirror :mod:`scitex_todo._index` (the existing
...
#: ``SCITEX_TODO_INDEX_PATH`` override on :mod:`scitex_todo._index`.
...
def open_connection(path: Optional[Path] = None):
    ...
    (first-run friendly). Mirrors :func:`scitex_todo._index.open_connection`.
    """
...
def enqueue(
    ...
    *,
    ...
    """SQLite twin of :func:`scitex_todo._inbox.enqueue` — same contract.
...
def poll_inbox(
    ...
    *,
    ...
    """SQLite twin of :func:`scitex_todo._inbox.poll_inbox` — same contract.
...
def ack(
    ...
    """SQLite twin of :func:`scitex_todo._inbox.ack` — same contract.
