...
The resolver half of foundation C3 (see :mod:`scitex_cards._notify`). Holds
...
def _resolve_name_to_id(name: str, *, store: str | Path | None) -> str:
    ...
    Reuses :func:`scitex_cards._users.resolve_user` as the SSOT for identity.
    When the raw name maps to no registered user, the name is first run
    through :func:`scitex_cards._users.canonical_identity` to collapse naming
...
def card_role_members(
    ...
    Roles (see :data:`~scitex_cards._notify._rules.VALID_ROLES`):
    ...
    * ``subscribers``   — the card ``subscribers`` list.
    ...
    ----------
    card : Mapping
        A task/card dict (the same shape :class:`scitex_cards._model.Task`
...
def _event_type_of(event: Any) -> str:
    ...
    Accepts a :class:`scitex_cards._events.Event` (``.type``) or a wire dict
...
def resolve_recipients(
    ...
    *,
    ...
    a. ``event_type`` from ``event["type"]`` (a dict) or ``event.type``
       (an :class:`~scitex_cards._events.Event`); validated against the
       taxonomy.
