...
| `SCITEX_TODO_CHANNEL_SOURCE` | `scitex-todo-system` | `mcp channel` `meta.source` (drives the `<- scitex-todo-system` render — deliberately distinct from the `scitex-todo` agent id). Overridden by `--name`. |
