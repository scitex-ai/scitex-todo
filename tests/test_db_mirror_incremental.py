...
def test_first_run_falls_back_to_a_full_rebuild(tmp_path):
    ...
    out = mirror_doc_incremental(_doc(_card("a"), _card("b")), db)
...
def test_a_changed_card_is_updated(tmp_path):
    ...
    got = {r["id"]: r["status"] for r in _rows(db, "tasks")}
    assert got["a"] == "done"
...
def test_comments_are_not_duplicated_on_re_write(tmp_path):
    ...
    db = _fresh_db(tmp_path)
    c = _card("a", comments=[{"author": "x", "ts": "2026-01-01", "text": "hello"}])
    mirror_doc_incremental(_doc(c), db)
    c2 = _card("a", status="done",
               comments=[{"author": "x", "ts": "2026-01-01", "text": "hello"}])
    mirror_doc_incremental(_doc(c2), db)
...
def test_incremental_comments_equal_a_full_rebuild(tmp_path):
    ...
    a = [(r["task_id"], r["text"]) for r in _rows(inc, "task_comments")]
    b = [(r["task_id"], r["text"]) for r in _rows(full, "task_comments")]
...
def test_any_field_change_is_detected(field):
    ...
    assert _card_hash(base) != _card_hash(edited)
