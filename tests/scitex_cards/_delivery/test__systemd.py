...
def test_explicit_exec_start_is_honoured(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path / "cfg"))
    result = _systemd.install_unit(exec_start="/opt/venv/bin/scitex-todo notifyd")
    assert result["exec_start"] == "/opt/venv/bin/scitex-todo notifyd"
    assert (
        "ExecStart=/opt/venv/bin/scitex-todo notifyd"
