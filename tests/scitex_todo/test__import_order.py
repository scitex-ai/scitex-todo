...
WHY THIS FILE EXISTS (2026-07-14): `import scitex_todo._store_write` raised
ImportError when it was the FIRST scitex_todo import in a fresh interpreter --

    ImportError: cannot import name '_git_autocommit_store' from partially
    initialized module 'scitex_todo._store_write' (most likely due to a
    circular import)

-- while `import scitex_todo._model, scitex_todo._store_write` worked fine.
...
was `from scitex_todo._store_write import edit_tasks`. The ImportError blamed a
...
def _discover_modules() -> list[str]:
    ...
    for path in sorted(PKG.glob("*.py")):
        if path.stem == "__init__":
            continue
        names.append(f"scitex_todo.{path.stem}")
    return names
...
def _import_first(statement: str) -> subprocess.CompletedProcess:
    """Run `statement` as the FIRST scitex_todo import in a clean interpreter."""
...
def test_module_can_be_imported_first(module):
    ...
    assert result.returncode == 0, (
        f"`import {module}` FAILS as the first scitex_todo import:\n"
        f"{result.stderr}\n"
...
def test_the_exact_regression_from_the_data_repair_script():
    ...
    result = _import_first(
        "from scitex_todo._store_write import edit_tasks, save_tasks; "
...
def test_lazy_reexports_are_still_reachable_through_model():
    ...
    43 test files and every fleet agent do `from scitex_todo._model import ...`.
    ...
    """
    from scitex_todo._model import _STORE_WRITE_EXPORTS

    for name in sorted(_STORE_WRITE_EXPORTS):
        result = _import_first(
            f"from scitex_todo._model import {name}; assert {name} is not None"
        )
        assert result.returncode == 0, (
            f"`from scitex_todo._model import {name}` broke -- the lazy re-export "
...
def test_reexported_object_is_the_same_object():
    ...
    import scitex_todo._model as model
    import scitex_todo._store_write as store_write
