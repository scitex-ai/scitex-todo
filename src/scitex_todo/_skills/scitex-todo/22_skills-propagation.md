...
``src/scitex_todo/_skills/manifest.yaml`` (resolved at runtime via the
:func:`scitex_todo._cli._skills_propagate.manifest_path` helper). It
...
- ``src/scitex_todo/_cli/_skills_propagate.py`` — implementation.
- ``src/scitex_todo/_skills/manifest.yaml`` — the manifest file
  itself.
