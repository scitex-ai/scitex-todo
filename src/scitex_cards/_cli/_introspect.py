...
ships a real MCP server (``scitex_cards._mcp_server``); the live ``mcp``
...
@click.command(
    "list-python-apis",
    **spec_command_kwargs(
        summary="List the public Python API (scitex_cards.__all__).",
        description=(
...
def list_python_apis_cmd(verbose: int, as_json: bool) -> None:
    """List the public Python API surface of scitex_cards."""
    import scitex_cards

    names = [n for n in scitex_cards.__all__ if n != "__version__"]
    entries = [_describe_api(scitex_cards, n) for n in names]
...
def register(group: click.Group) -> None:
    ...
    :mod:`scitex_cards._cli._mcp` (real Phase-1 server). Importing this
