...
/scitex-cards
$ readlink -f /scitex-cards
/scitex-cards
```

So `/home/agent/.scitex/todo` is a symlink → `/scitex-cards`. Same inode:

```text
$ stat -c "%i %n" /scitex-cards/tasks.yaml /home/agent/.scitex/todo/tasks.yaml
14212385 /scitex-cards/tasks.yaml
14212385 /home/agent/.scitex/todo/tasks.yaml
$ md5sum /scitex-cards/tasks.yaml /home/agent/.scitex/todo/tasks.yaml
d92dc26066610811024f0e79e1ce3a2b  /scitex-cards/tasks.yaml
...
```text
8:48 /home/ywatanabe/.dotfiles/src/.scitex/todo  /scitex-cards  rw,nosuid,nodev,relatime ...
8:48 /home/ywatanabe/proj/scitex-cards            /work         rw,nosuid,nodev,relatime ...
8:48 /home/ywatanabe/.dotfiles/src/.scitex/agent-container/runtime/proj-scitex-cards  /state/proj-scitex-cards  rw,nosuid,nodev,relatime ...
```

The container's `/scitex-cards` is bound from
...
```text
$ /opt/venv-agent/bin/scitex-cards resolve-store
...
by project (top):                by assignee (top):
   ...
   83  scitex-agent-container         62  proj-scitex-cards
   80  scitex-cards                     8  proj-scitex-hub
   50  <none>                          5  lead
...
```text
$ git -C /scitex-cards remote -v
(no remotes)
$ git -C /scitex-cards log --format="%h %ai" -5
...
So today there is no mechanism in scitex-cards itself that would keep two
...
> stale snapshot of the dotfiles tree) than this proj-scitex-cards
...
Secondary contributors (ranked):
  ...
  - (c) **project-scope override**: per `_paths.py`, precedence-3
    ...
    (`/work` = scitex-cards repo, no `.scitex/todo/` directory) but
...
|------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------|--------|
| 1    | **Realign bind source with the P3a-2 spec.** Make the host's `~/.scitex/todo` a *real symlink* into `~/.dotfiles/src/.scitex/todo`, OR change the dotfiles `_base/` spec to bind the actual host target the runtime uses. Add a startup probe (`scitex-cards verify-store`) that fails loudly when the resolved store's inode disagrees with an operator-pinned fingerprint. | LOW  | small  |
...
| 3    | **Hub-only triage**: enter the proj-scitex-hub container, run `scitex-cards resolve-store` + `cat /proc/self/mountinfo | grep todo` + `md5sum` to confirm exactly which file it is reading. Patch *that* container's bind directly without touching the design. Tactical, not strategic — buys time for option 1 or 2.                  | LOW  | tiny   |
...
**Owner**: proj-scitex-cards lead (architecture) + operator
(host-side dotfiles).

**Exact follow-up PR(s)** suggested:

  - `chore(p3a-2): pin tasks.yaml bind source — symlink + verify-store
    probe` (Option 1). Adds a `scitex-cards verify-store` subcommand
