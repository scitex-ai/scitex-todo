...
def _script_import_root(path: Path) -> str | None:
    ...
    for line in text.splitlines():
        ...
        for prefix in ("from ", "import "):
            ...
            if root in (_REQUIRED_IMPORT_ROOT, _SUPERSEDED_IMPORT_ROOT):
                return root
    return None
...
def check_console_scripts_not_shadowed() -> dict[str, object]:
    ...
    That container had scitex-cards 0.17.5 correctly installed, and every
    version check said so — including :func:`check_install_honest`, which
    compares metadata against the code beside it and was entirely right. But
    ``bin/scitex-cards`` was a 224-byte shim importing ``scitex_todo._cli``
    from the superseded 0.13.5 distribution, because BOTH distributions
    declare console scripts of the same names and the last install owns the
    name. 0.13.5 predates the SQLite backend, so it ignored
    ``SCITEX_CARDS_STORE_BACKEND`` and fell through YAML precedence to the
    BUNDLED EXAMPLE inside site-packages: that agent read 17 fixture rows
    where the board had 2,308, and its writes landed in a package file
    nothing reads and any reinstall erases. It went unnoticed until someone
    noticed the row count was absurd.
    ...
    beside it, and the import graph really does contain the right package. The
    only thing wrong is which module the SCRIPT on ``$PATH`` reaches for, so
    that is what this reads — 308 bytes importing ``scitex_cards._cli`` is
    healthy, 224 bytes importing ``scitex_todo._cli`` is the failure.

    Complementary to :func:`check_install_honest`, not a superset: that one
    catches metadata outliving its code, this one catches a correct install
    being bypassed. Neither sees the other's class.

    NOT a failure when no script is found: this package is legitimately used
    as a library and through the MCP server, where nothing needs a script on
    ``$PATH``. "Absent" and "pointing at the wrong package" are different
    facts and only the latter is damage.
    """
    shadowed: list[str] = []
    indeterminate: list[str] = []
    ours: list[str] = []
    for name in _OWNED_SCRIPTS:
        found = shutil.which(name)
        if not found:
            continue
        root = _script_import_root(Path(found))
        # ALWAYS name the target that was found, never just a verdict: this
        # check exists because a boolean ("install is fine") is exactly what
        # hid the incident. The reader must be able to see WHICH module the
        # script reaches for.
        if root == _SUPERSEDED_IMPORT_ROOT:
            shadowed.append(f"{found} -> imports {root}")
        elif root == _REQUIRED_IMPORT_ROOT:
            ours.append(f"{name} -> imports {root}")
        else:
            ...
            indeterminate.append(f"{found} -> import target undetermined")
    if shadowed:
        return {
            "ok": False,
            "detail": (
                ...
                "every version check reports a healthy, current install."
            ),
            "hint": (
                "reinstall so this package owns the scripts again: "
                "`pip install --force-reinstall --no-deps scitex-cards`, then "
                "re-run this check. If it comes back, the stale script is in "
                "a container overlay upper, which permanently shadows the base "
    ...
    if indeterminate:
        return {
            ...
            "detail": (
                ...
                "how the 2026-07-22 shim stayed invisible."
            ),
            "hint": (
                "read the script yourself and confirm it imports "
                f"{_REQUIRED_IMPORT_ROOT}; if it is a wrapper this check does "
    ...
    if not ours:
        return {
            ...
            "detail": (
                ...
                "(library/MCP-only install)"
            ),
            "hint": None,
        }
    return {
        "ok": True,
        "detail": "console script(s) verified: " + "; ".join(ours),
        "hint": None,
    }
