...
These helpers wrap :func:`scitex_todo._users.is_alive` /
:func:`scitex_todo._users.resolve_user` and stay fail-soft — a registry
lookup error must NEVER break a sweep (an unresolvable owner is a dead owner
for escalation purposes). Extracted from :mod:`scitex_todo._reminders` (the
...
def _owner_liveness(
    ...
    *,
    ...
    :func:`scitex_todo._users.is_alive` dict. A resolution error must NEVER
